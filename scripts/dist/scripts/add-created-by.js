...
async function addCreatedByField() {
    ...
    try {
        ...
        const allCollections = Object.values(Question_1.COLLECTIONS);
        let totalUpdated = 0;
        for (const collectionName of allCollections) {
            try {
                ...
                // Add createdBy field with value "Luis" to all documents that don't have it
                const result = await collection.updateMany({ createdBy: { $exists: false } }, // Only update documents without createdBy field
                { $set: { createdBy: 'Luis' } });
                console.log(`Updated ${result.modifiedCount} documents in ${collectionName} collection`);
                totalUpdated += result.modifiedCount;
                // Create index on createdBy field for better query performance
                await collection.createIndex({ createdBy: 1 });
                console.log(`Created index on createdBy field for ${collectionName} collection`);
            }
            catch (error) {
        ...
        console.log('Successfully added createdBy field to all collections!');
    }
    catch (error) {
...
exports.default = addCreatedByField;
